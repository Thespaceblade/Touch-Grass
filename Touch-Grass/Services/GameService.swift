...
final class GameService: ObservableObject {
    ...
    private let bluetoothTagService = BluetoothTagService()
    
    // Location update throttling
    private var lastFirestoreUpdate: Date?
    private let firestoreUpdateInterval: TimeInterval = 3.0 // Update Firestore every 3 seconds max (optimized from 2s)
    ...
    init(locationService: LocationService) {
        ...
        // Setup BLE callbacks
        bluetoothTagService.onTagRequest = { [weak self] fromPlayerId, fromPlayerName in
            Task { @MainActor in
                self?.handleTagRequest(fromPlayerId: fromPlayerId, fromPlayerName: fromPlayerName)
            }
        }
        
        bluetoothTagService.onTagConfirmed = { [weak self] playerId in
            Task { @MainActor in
    ...
    func createSession(hostName: String, hostLocation: CLLocationCoordinate2D, gameType: GameType = .manhunt) {
    ...
    private func validateBubble(_ bubble: Bubble) -> Bool {
        ...
        // For CTF: duration can be infinite (no time limit)
        // For other games: duration must be finite and within limits
        if bubble.duration.isInfinite {
            // Infinite duration is only valid for CTF (no time limit, no shrinking)
            // This is handled in configureGame, so we allow it here
            guard bubble.shrinkInterval.isInfinite else {
                return false // If duration is infinite, shrinkInterval should also be infinite
            }
        } else {
            // Finite duration validation (for non-CTF games)
            guard bubble.duration > 0 && bubble.duration.isFinite else {
                return false
            }
            guard bubble.duration <= 3600 else { // Max 1 hour
                return false
            }
    ...
    func configureGame(bubble: Bubble, hunterCount: Int? = nil, scoreLimit: Int? = nil, teamABase: CLLocationCoordinate2D? = nil, teamBBase: CLLocationCoordinate2D? = nil) {
        ...
        // Validate hunter count if provided
    ...
    func beginGame() {
        ...
        guard session.players.count >= 1 else {
                let error = "Cannot begin game: Need at least 1 player to start. Currently \(session.players.count) player(s)."
                print("❌ \(error)")
                beginGameError = error
                beginGameErrorAction = .openSessionSetup
            // Ensure game state stays in lobby
        ...
        guard session.hunterCount >= 0 else {
            let error = "Cannot begin game: Invalid hunter count (\(session.hunterCount))."
            print("❌ \(error)")
            beginGameError = error
            // Ensure game state stays in lobby
            if gameState != .lobby {
        ...
        // Skip this check for CTF (uses teams instead)
        if session.gameType != .captureTheFlag {
            ...
            guard hiderCount >= 0 else {
                let error = "Cannot begin game: Invalid player configuration. You have \(session.players.count) player(s) but \(session.hunterCount) hunter(s), which would result in \(hiderCount) hider(s)."
                print("❌ \(error)")
                beginGameError = error
                // Ensure game state stays in lobby
                if gameState != .lobby {
        ...
        // Reset bubble start time to now when game actually begins
        ...
        // Request notification permissions for CTF
        ...
        print("✅ Bubble start time reset to: \(bubble.startTime)")
    ...
    private func assignRandomHunters(session: inout GameSession, count: Int) {
        ...
        // TEMPORARY: Allow 0 hunters for testing
        guard count >= 0 && count <= session.players.count else {
            print("⚠️ Invalid hunter count: \(count), defaulting to 0 (testing mode)")
            // Default to 0 hunters if invalid (all hiders for testing)
            for i in 0..<session.players.count {
    ...
    func placeSafeZone(team: Flag.Team, center: CLLocationCoordinate2D, radius: Double) {
        ...
        // Create safe zone
        let safeZone = GameSession.SafeZone(center: center, radius: radius)
        
        // Set safe zone for team
        if team == .teamA {
        ...
        Task {
            do {
                try await firestoreService.updateSession(session)
                print("✅ \(team.rawValue) safe zone placed at \(center.latitude), \(center.longitude) with radius \(radius)m")
            } catch {
    ...
    func endGame() {
        ...
        var stats = gameStats ?? GameStats(gameStartTime: Date())
    ...
    func joinGame(joinCode: String, playerName: String, playerLocation: CLLocationCoordinate2D, expectedGameType: GameType? = nil) async -> (Bool, String?) {
        ...
        do {
            ...
            // Allow joining even if game has started (for reconnection or late joiners)
            // Just log a warning
            if session.gameState != .lobby {
                print("⚠️ Joining game that has already started (state: \(session.gameState.rawValue))")
            }
            ...
            var updatedSession = session
            if let existingPlayerIndex = updatedSession.players.firstIndex(where: { $0.id == currentPlayer?.id ?? "" }) {
    ...
    func updatePlayerLocation(_ coordinate: CLLocationCoordinate2D) {
        ...
        // Throttle Firestore updates (only sync every 3 seconds)
        let now = Date()
        if let lastUpdate = lastFirestoreUpdate {
            let timeSinceLastUpdate = now.timeIntervalSince(lastUpdate)
            if timeSinceLastUpdate >= firestoreUpdateInterval {
    ...
    private func checkForDisconnectedPlayers(session: GameSession) {
        var session = session
        let now = Date()
        let disconnectTimeout: TimeInterval = 30.0 // 30 seconds
        let reconnectionWindow: TimeInterval = 120.0 // 2 minutes to reconnect
        ...
        if session.gameType == .captureTheFlag {
            ...
            if let teamAFlag = session.players.first(where: { $0.isFlag && $0.team == .teamA }) {
                ...
                if isDisconnected && flagPhoneDisconnected[.teamA] != true {
                    ...
                    // TODO: Implement notification service
                    // Task {
                    //     await notificationService.notifyFlagDisconnected(team: .teamA)
                    // }
                    ...
                    // TODO: Implement notification service
                    // Task {
                    //     await notificationService.notifyFlagReconnected(team: .teamA)
                    // }
                }
            }
            ...
            if let teamBFlag = session.players.first(where: { $0.isFlag && $0.team == .teamB }) {
                ...
                if isDisconnected && flagPhoneDisconnected[.teamB] != true {
                    ...
                    // TODO: Implement notification service
                    // Task {
                    //     await notificationService.notifyFlagDisconnected(team: .teamB)
                    // }
                    ...
                    // TODO: Implement notification service
                    // Task {
                    //     await notificationService.notifyFlagReconnected(team: .teamB)
                    // }
                }
            }
    ...
    private func checkOutOfBounds() {
        ...
        // Validate bubble radius is valid
        ...
        let distance = bubble.distanceToEdge(from: player.coordinate, at: Date())
        ...
        if distance > 0 {
            ...
            isOutOfBounds = false
            updateWarningLevel(distance: abs(distance))
        }
    }
    
    private func updateWarningLevel(distance: Double) {
        guard let bubble = session?.bubble else { return }
        let radius = bubble.currentRadius(at: Date())
        let warningZone = radius * 0.2 // 20% of radius
    ...
    private func alertFlagMovement(flagPlayer: Player, isMoving: Bool, session: GameSession) {
        ...
        //         sound: nil // Silent for motion alerts
        //     )
        // }
    ...
    func capturePlayerFlag(flagPlayerId: String) {
        ...
        //     await notificationService.notifyFlagCaptured(
    ...
    func takePossessionOfFlag(flagPlayerId: String) {
        ...
        // Transfer flag possession to defender
        session.flagCarriers[flagPlayerId] = currentPlayer.id
    ...
    func returnPlayerFlag(flagPlayerId: String) {
        ...
        //     await notificationService.notifyFlagReturned(
        //         team: flagTeam,
    ...
    private func handleTagRequest(fromPlayerId: String, fromPlayerName: String) {
        // Check if this is a valid tag request
        guard let session = session,
              let currentPlayer = currentPlayer,
              currentPlayer.role == .hider,
              currentPlayer.isAlive,
              let fromPlayer = session.players.first(where: { $0.id == fromPlayerId }),
              fromPlayer.role == .hunter else {
            print("❌ Invalid tag request: not from a hunter or you're not a hider")
            return
        }
    ...
    private func eliminatePlayer(_ playerId: String) {
        ...
        // Handle flag player leaving before elimination (for other game types)
        if session.gameType == .captureTheFlag {
            handleFlagPlayerLeaving(leavingPlayerId: playerId, session: &session)
            self.session = session
        }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
            self?.lastEliminationMessage = nil
        }
    }
    ...
    private func catchPlayer(_ playerId: String) {
        ...
        if isZombieTag {
            ...
            // Manhunt: Hunter catches hider
            guard let hunter = tagger,
              hunter.role == .hunter else { return }
        
        // Edge case: Cannot catch yourself
        guard playerId != hunter.id else {
            print("⚠️ Attempted to catch self - ignoring")
            return
        }
        
        // Edge case: Cannot catch other hunters (only hiders can be caught)
        guard player.role == .hider else {
            print("⚠️ Attempted to catch hunter \(player.displayName) - only hiders can be caught")
            return
        }
        
        // Edge case: Cannot catch already eliminated players
        guard player.isAlive else {
        ...
        // CTF: If a flag player is tagged, drop the flag
        if session.gameType == .captureTheFlag && player.isFlag {
            session.flagCarriers[playerId] = nil
            print("🚩 Flag player \(player.displayName) was tagged - flag dropped!")
        }
        ...
        if var stats = gameStats, let tagger = tagger {
            ...
            // Record survival time for caught player
        ...
        // Set catch message for UI feedback
        if isZombieTag {
    ...
    private func startUpdateTimer() {
        stopUpdateTimer()
        // Optimized: Update every 2 seconds instead of 1s (reduces CPU usage by ~50%)
        updateTimer = Timer.scheduledTimer(withTimeInterval: 2.0, repeats: true) { [weak self] timer in
    ...
    // Check if a shrink event should occur and apply it
    ...
    func checkGameOver() {
        ...
        // TEMPORARY: Skip game over checks for solo testing (1 player)
        // This allows testing the map view without the game ending immediately
        ...
        if session.gameType == .captureTheFlag {
            ...
            guard bubble.duration.isFinite && bubble.duration > 0 else {
        ...
        if session.gameType == .zombieTag {
            ...
            // Check if time is up (humans win if time runs out)
            if elapsed >= bubble.duration {
    ...
    private func startSessionListener(_ sessionId: String) {
        ...
        firestoreService.listenToSession(sessionId) { [weak self] updatedSession in
            ...
            guard var session = updatedSession else {
                print("⚠️ Session not found in Firestore - may have been deleted")
            ...
            guard !self.isUpdatingSession else {
                print("⏸️ Ignoring Firestore update - local update in progress")
                return
            }
            
            // Update local session with Firestore data
            print("📡 Session updated from Firestore - \(session.players.count) players")
            for player in session.players {
                print("   - \(player.displayName) (\(player.role.rawValue)) at (\(player.latitude), \(player.longitude))")
            }
               ...
               let previousSession = self.session {
                ...
                for leftPlayerId in leftPlayerIds {
                    if let leftPlayer = previousSession.players.first(where: { $0.id == leftPlayerId }),
                       leftPlayer.isFlag {
                        handleFlagPlayerLeaving(leavingPlayerId: leftPlayerId, session: &session)
                    }
                }
            }
            
            // Preserve bubble from local session if Firestore update doesn't have it yet
            // This prevents the bubble from being cleared if Firestore update arrives before local update completes
            if let previousSession = self.session,
               let previousBubble = previousSession.bubble,
               session.bubble == nil,
               session.gameState == .lobby {
                print("🔒 Preserving local bubble in session listener (Firestore update doesn't have it yet)")
            ...
            if let playerId = self.currentPlayer?.id,
               let player = session.players.first(where: { $0.id == playerId }) {
                ...
                if !wasAlive && player.isAlive {
                    print("🔄 Player reconnected and is alive - restarting location updates")
                ...
                print("⚠️ Current player \(playerId) not found in session - may need to rejoin")
            }
            
            // Start BLE if game just became active (for players who joined or host)
            if previousGameState != .active && session.gameState == .active {
                print("🎮 Game became active - starting BLE for current player")
                if let playerId = self.currentPlayer?.id, let playerName = self.currentPlayer?.displayName {
                    self.bluetoothTagService.start(playerId: playerId, playerName: playerName)
                    print("✅ BLE started for \(playerName)")
                }
            }
            
            // Stop BLE if game ended
            if previousGameState == .active && session.gameState != .active {
                print("🛑 Game ended - stopping BLE")
