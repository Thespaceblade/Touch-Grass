...
struct ZombieTagCountdownView: View {
    @ObservedObject var gameService: GameService
    @ObservedObject var locationService: LocationService
    let onCountdownComplete: () -> Void
    
    @State private var timeRemaining: TimeInterval = 120.0 // 2 minutes
    ...
    // Hardcoded Zombie Tag theme properties
            ...
            if showGoScreen {
                ...
                if let currentPlayer = gameService.currentPlayer {
                    VStack(spacing: AppSpacing.sm) {
                        ...
                        HStack(spacing: AppSpacing.sm) {
                            ...
                            Text(currentPlayer.role == .zombie ? "ZOMBIE" : "HUMAN")
                ...
                VStack(spacing: AppSpacing.xs) {
                    if let role = gameService.currentPlayer?.role {
                        if role == .zombie {
                            instructionText("Infect all humans to win")
                            instructionText("Use Bluetooth to infect nearby players")
                        } else {
                            instructionText("Survive and avoid the zombies")
